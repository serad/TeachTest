class Course {
	...
	save() {
		...
		if(error) { Materialize.toast('Check form', 4000, 'red'); return false; }
		$.post('/course/save', { course: this }, function( data ) {
			if(data.result == 'ok' ){
	...
	initForm() {
		...
		courseName.change( function( elem ) { that.courseName = elem.target.value; } );

		var definition = $('#definition');
		definition.val( this.definition );
		definition.change( function( elem ) { that.definition = elem.target.value; } );
		var dateStart = $('#date_start');
		if( typeof this.startDate != 'undefined' )
		dateStart.pickadate('picker').set('select', this.startDate ? new Date( this.startDate ) : '' );
		dateStart.change( function ( elem ) { that.startDate = $(elem.target).pickadate('picker').get('select', 'yyyy-mm-dd'); } );
		...
		dateEnd.change( function ( elem ) { that.endDate = $(elem.target).pickadate('picker').get('select', 'yyyy-mm-dd'); } );
	
		var level = $('#level')
		level.val( this.level );
		level.change( function ( elem ) { that.level = elem.target.value } );
		$('#level').material_select();

		var active = $('#active');
		active.prop('checked', parseInt(this.active));
		active.change( function( elem ) { that.active = elem.target.checked ? 1 : 0; } );
		
		$( "input" ).change(function( elem ) { $(elem.target).removeClass('invalid'); });
		$( "select" ).change(function ( elem ) { $(elem.target).parent().removeClass('invalid'); } )
	}
